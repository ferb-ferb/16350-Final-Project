...
int main(int argc, char *argv[])
{
    ...
    std::unordered_map<int, Path> plan = CBSPlanner(map, agents);
    ...
    for (int i = 0; i< num_robots; i++)
    {
        ...
        {
            robotPoseX = curr_loc.get_X();
            robotPoseY = curr_loc.getY();
            output_file << i << "," << curr_step << "," << robotposeX << "," << robotposeY << std::endl;
            curr_step += 1;
        }
        printf("\n");
    }
